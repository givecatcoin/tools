...
impl Store {
    ...
    // ============================================================================
    // 既存ストアへ config.json だけ書く。
    // 既にあれば force 無しではエラー。objects / snapshots は触らない。
    // ============================================================================
    pub fn init_config_only(
        target_path: &Path,
        ...
        let root = match store_path {
            Some(path) => {
                let resolved = resolve_store_location(&target, Some(path))?;
                if !resolved.is_dir() {
                    bail!(
                        "store does not exist: {}; run `snapline init` first",
                        resolved.display()
                    );
                }
                resolved
            ...
            None => locate_store_from_tree(&target).with_context(|| {
                format!(
                    "no existing store under {}; run `snapline init` first",
                    target.display()
                )
            })?,
        };
        ...
        if config_path.exists() && !force {
            bail!(
                "config.json already exists at {}; re-run with --force to overwrite",
                config_path.display()
            );
        }
        ...
        write_json_atomic(&config_path, &config, &root.join("tmp"))?;
        Ok(Self { root, config })
    }
    ...
    // ============================================================================
    // 作成時刻が最も新しいマニフェストを返す。履歴が無ければ None。
    // 要約があればそれで時刻比較し、無ければマニフェスト本体を読む。
    // ============================================================================
    pub fn latest_manifest(&self) -> Result<Option<SnapshotManifest>> {
        let paths = self.snapshot_manifest_paths()?;
        let mut best: Option<(String, String)> = None;
        for path in paths {
            let Some(id) = path.file_stem().and_then(|value| value.to_str()) else {
                continue;
            };
            let created_at = match self.read_summary(id) {
                Ok(summary) => summary.created_at,
                Err(_) => {
            ...
            let replace = match &best {
                None => true,
                Some((previous, _)) => created_at > *previous,
            };
            if replace {
                best = Some((created_at, id.to_owned()));
            }
        }

        match best {
            Some((_, id)) => Ok(Some(self.read_manifest(&id)?)),
            None => Ok(None),
        }
    }
    ...
    pub fn lock(&self) -> Result<StoreLock> {
            ...
            .open(&path)
        {
            Ok(mut file) => {
                ...
                file.sync_all()?;
                // ロック取得直後は自プロセスの一時ファイルはまだ無い。残骸だけ消す。
            ...
            Err(error) if error.kind() == io::ErrorKind::AlreadyExists => {
                ...
                let lock_path = display_path(&path);
                match holder {
                    Some(detail) => bail!(
                        "store is locked ({detail} in {lock_path}). Stop that snapline process, or remove the lock file if it is stale."
                    ),
                    None => bail!(
                        "store is locked ({lock_path}). Stop the other snapline process, or remove the lock file if it is stale."
                    ),
                }
            }
...
// ============================================================================
// ユーザー向けメッセージ用。Windows の \\?\ プレフィックスは除く。
// ============================================================================
...
mod tests {
    ...
    // ============================================================================
    // ロック取得済みのとき、英語だけの明確なエラーになることを確認する。
    // ============================================================================
    #[test]
    fn lock_reports_clear_error_when_already_held() -> Result<()> {
        let root = tempfile::tempdir()?;
        let target = root.path().join("workspace");
        fs::create_dir(&target)?;
        let store = Store::init(&target, None)?;
        let _first = store.lock()?;
        let error = match store.lock() {
            Ok(_) => panic!("second lock must fail"),
            Err(error) => error,
        };
        let text = format!("{error:#}");
        assert!(text.contains("store is locked"));
        assert!(text.contains("write.lock"));
    ...
    // ============================================================================
    // write.lock 取得時に tmp 残骸が消えることを確認する。
    // ============================================================================
    #[test]
    fn lock_cleans_stale_tmp_files() -> Result<()> {
        let root = tempfile::tempdir()?;
        let target = root.path().join("workspace");
        fs::create_dir(&target)?;
        let store = Store::init(&target, None)?;
        let junk = store.root.join("tmp").join("stale.bin");
        fs::write(&junk, b"leftover")?;
        assert!(junk.is_file());
    ...
    // ============================================================================
    // config 欠落時は --config-only で復元でき、既存時は --force が必要なことを確認する。
    // ============================================================================
    #[test]
    fn init_config_only_requires_force_to_overwrite() -> Result<()> {
        let root = tempfile::tempdir()?;
        let target = root.path().join("workspace");
        fs::create_dir(&target)?;
        let store = Store::init(&target, None)?;
        ...
        let written = Store::init_config_only(&target, None, false)?;
        assert!(config_path.is_file());
        assert_eq!(written.config.target, target.canonicalize()?);

        match Store::init_config_only(&target, None, false) {
            Ok(_) => panic!("must require force"),
            Err(error) => assert!(error.to_string().contains("--force")),
        }
        Store::init_config_only(&target, None, true)?;
        assert!(config_path.is_file());
        Ok(())
    }
