...
// ============================================================================
// マニフェスト一覧の結果。壊れて読めなかったスナップは skipped に残す。
// ============================================================================
...
pub fn list_with_progress(
    ...
    for (index, path) in paths.iter().enumerate() {
        ...
        match read_json::<SnapshotManifest>(path) {
            Ok(manifest) => match validate_manifest(&manifest) {
                Ok(()) => listed.manifests.push(manifest),
                Err(error) => {
                    progress.note(&format!(
                        "skipping unreadable snapshot manifest {label}: {error}"
                    ));
                    listed.skipped.push(label.to_owned());
                }
            },
            Err(error) => {
                progress.note(&format!(
    ...
    if paths.is_empty() {
        ...
        progress.done(&format!(
            "{} readable, {} skipped.",
            listed.manifests.len(),
            listed.skipped.len()
        ));
    }
    listed
        .manifests
        .sort_by(|left: &SnapshotManifest, right| left.created_at.cmp(&right.created_at));
    Ok(listed)
}
...
pub fn ensure_summaries(store: &Store, progress: &mut Progress) -> Result<usize> {
    ...
    // 要約書き込みはストア変更なので排他する。
    progress.begin("Acquiring write lock");
    ...
    for (index, (path, id)) in missing.iter().enumerate() {
        ...
        let file: SnapshotLogFile = match read_json(path) {
            Ok(file) => file,
            Err(error) => {
                progress.note(&format!(
                    "skipping unreadable snapshot {id} while building summary: {error}"
                ));
                continue;
            }
        };
        ...
        if summary.id != *id {
            progress.note(&format!(
                "skipping snapshot {id}: file name does not match id field {}",
                summary.id
            ));
            continue;
        }
...
// ============================================================================
#[allow(dead_code)]
pub fn verify(store: &Store, progress: &mut Progress) -> Result<VerifyOutcome> {
...
pub fn verify_with_pace(
    ...
    let listed = list_with_progress(store, progress)?;
...
mod tests {
    ...
    use super::{SnapshotLogFile, ensure_summaries, list_log_rows, list_with_progress, verify};
    use crate::{
    ...
    // ============================================================================
    // オブジェクト破損時に verify が失敗することを確認する。
    // ============================================================================
    #[test]
    fn verify_fails_when_object_payload_is_corrupted() -> Result<()> {
        ...
        let error = verify(&store, &mut Progress::quiet()).expect_err("verify must fail");
        assert!(
    ...
    // ============================================================================
    // 壊れたマニフェストがあっても他スナップの検証が続くことを確認する。
    // ============================================================================
    #[test]
    fn verify_skips_broken_manifest_and_continues() -> Result<()> {
        let root = tempfile::tempdir()?;
        let target = root.path().join("tree");
        fs::create_dir_all(&target)?;
        fs::write(target.join("note.txt"), "hello")?;
        let store = Store::init(&target, None)?;
        snapshot::create(&store, None)?;
        fs::write(
            store.root.join("snapshots").join("zzzz-broken0001.json"),
            "{not-json",
        )?;

        let outcome = verify(&store, &mut Progress::quiet())?;
        assert_eq!(outcome.snapshots, 1);
        assert!(outcome.objects >= 1);
        assert_eq!(outcome.skipped.len(), 1);
        assert!(outcome.skipped[0].contains("zzzz-broken0001"));
        // スキップした事実は結果に残り、成功扱いにはしない（呼び出し側で拒否する想定）。
        assert!(!outcome.skipped.is_empty());
        Ok(())
    }

    // ============================================================================
    // 壊れたマニフェストがあっても、読めたスナップは一覧から消えないことを確認する。
    // ============================================================================
    #[test]
    fn list_keeps_readable_manifests_when_another_is_broken() -> Result<()> {
        let root = tempfile::tempdir()?;
        let target = root.path().join("tree");
        fs::create_dir_all(&target)?;
        fs::write(target.join("note.txt"), "hello")?;
        let store = Store::init(&target, None)?;
        let created = snapshot::create(&store, None)?;
        fs::write(
            store.root.join("snapshots").join("zzzz-broken0002.json"),
            "{not-json",
        )?;

        let listed = list_with_progress(&store, &mut Progress::quiet())?;
        assert_eq!(listed.manifests.len(), 1);
        assert_eq!(listed.manifests[0].id, created.manifest.id);
        assert_eq!(listed.skipped.len(), 1);
        assert!(listed.skipped[0].contains("zzzz-broken0002"));
        Ok(())
    }
