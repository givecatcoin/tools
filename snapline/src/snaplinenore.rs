//! ============================================================================
//! `.snaplinenore` による除外ルール。
//! `.gitignore` と同じ記法で、各ディレクトリに置ける。
//! 祖先から当該ディレクトリまでのすべての `.snaplinenore` を重ねて適用する。
//! ============================================================================
...
// ============================================================================
// 対象ツリー内の `.snaplinenore` を遅延ロードし、パス判定に使う。
// ============================================================================
...
impl SnaplinenoreMatcher {
    ...
    // ============================================================================
    // 指定ディレクトリの `.snaplinenore` を読み、無ければ None をキャッシュする。
    // ============================================================================
    fn ensure_layer(&mut self, dir: &Path) -> Result<()> {
        ...
        let file = dir.join(SNAPLINENORE_FILE);
...
mod tests {
    ...
    // ============================================================================
    #[test]
    fn applies_nested_snaplinenore_files() -> Result<()> {
        let root = tempfile::tempdir()?;
        let target = root.path().join("tree");
        fs::create_dir_all(target.join("project/logs"))?;
        fs::write(target.join(".snaplinenore"), "*.tmp\n")?;
        fs::write(target.join("project/.snaplinenore"), "logs/\n")?;
