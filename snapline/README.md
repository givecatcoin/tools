...
記録します。同一内容は 1 回だけ保存します。普段の `snap` は無圧縮で取り込み、
`snap --compress` と `care` のときだけ効く場合に Zstandard 圧縮します。

## スナップショットの包含・除外（重要）

Snapline の `snap` は、**明示された除外以外を落とさない**ことを方針とします。
...
`C:\work` で `snapline snap` すると、子の `app\.snapline` も履歴に含まれます。
...
Snapline が提供するコマンドは `init`、`snap`、`log`、`tree`、`find`、`restore`、`verify`、`care`、`config`、`install` です。

```text
普段:    snapline snap -m "メモ"
たまに:  snapline care
必要時:  snapline snap --rehash
         snapline snap --compress
         snapline snap --rehash --compress
```
...
| オプション | 環境変数 | 意味 |
| --- | --- | --- |
| `--target <PATH>` | `SNAPLINE_TARGET` | 対象ツリー。省略時はカレントから親方向へ `.snapline` を探す |
| `--store <PATH>` | `SNAPLINE_STORE` | ストア配置先。省略時は `<target>/.snapline` |
| `--background` | （なし） | `snap` / `care` / `restore` / `verify` を低優先度・資源監視付きで実行 |

`--target` を省略したときは、カレントディレクトリから親へ順にたどり、
...
snapline --target C:\work --store C:\stores\work init

# 既存ストアの config.json だけ作り直す（履歴本体は触らない）
# 既に config.json がある場合はエラー。上書きするときだけ --force
snapline init --config-only
snapline init --config-only --force
```

### `snap` — 現在のツリーを記録する

```powershell
snapline --target C:\work snap
snapline --target C:\work snap -m "Before migration"

# 対象ツリー内のサブフォルダでも --target を省略できる
Set-Location C:\work\projectA\src
snapline snap -m "daily"

# 全ファイルを読み直してハッシュし直す（圧縮はしない）
snapline --target C:\work snap --rehash

# 変わったファイルだけ圧縮して保存（簡易整合は維持）
snapline --target C:\work snap --compress

# 全読込 + 圧縮
snapline --target C:\work snap --rehash --compress
```

既定の `snap` は **無圧縮** で、直前スナップショットと **size / mtime が一致**しオブジェクトの簡易整合（存在＋ヘッダ）が通ったファイルは内容を再読込しません。
完了時に、再利用件数・今回新規に取り込んだ容量（`+… new`）・所要時間を表示します。

| オプション | 取り込み | 保存 |
| --- | --- | --- |
| （なし） | 簡易整合 | raw |
| `--rehash` | 全ファイルを読んでハッシュし直す | raw |
| `--compress` | 簡易整合 | 新規・変更分を zstd レベル 3（効かなければ raw） |
| `--rehash --compress` | 全読込 | zstd レベル 3（効かなければ raw） |

内容が size/mtime を変えずに差し替わる改変は、既定／`--compress` 経路では検出できません（`--rehash` なら検出できる）。
オンディスク形式（`SNAPOBJ1` / 旧ヘッダ無し）は維持し、過去ストアはそのまま読めます。

### `log` — スナップショット一覧

```powershell
snapline --target C:\work log

# 最新だけ表示
snapline --target C:\work log -1
snapline --target C:\work log -n 1
```
...
旧ストアに要約が無い場合は、`log`（または次の `snap`）実行時にマニフェストから自動生成します。

### `tree` — スナップ内のパスをツリー表示する

```powershell
snapline tree a1b2c3d4e5f6
snapline tree a1b2c3d4e5f6 --path docs
snapline tree a1b2c3d4e5f6 --path docs --depth 2
```

復元前の下調べ用です。末尾にエントリ数とファイル論理サイズの合計を出します。

### `find` — スナップ内パスを検索する

```powershell
snapline find a1b2c3d4e5f6 readme
```

パス文字列の部分一致（大小無視）です。ヒット件数とファイルサイズ合計も出します。
...
# 一部だけ展開（上書きはしない。宛先は新規または空のみ）
snapline restore a1b2c3d4e5f6 D:/restored-docs --path docs

# 書き込まず、件数と想定展開容量だけ確認する
snapline restore a1b2c3d4e5f6 D:/restored-docs --path docs --dry-run
```

完全 ID、完全 ID の先頭、UUID 部分の先頭のいずれでも指定できます。
短縮 ID は 4 文字以上必要で、複数の履歴に一致する場合は安全のため拒否します。
復元先は新規または空ディレクトリのみです。既存ツリーは上書きしません。
実行時（および `--dry-run`）に、展開対象のエントリ数とファイル論理サイズ合計を表示します。

### `verify` — 履歴の整合性を確認する

```powershell
snapline --target C:\work verify
```

ストア内の全スナップショットと参照オブジェクトを検証します（展開して内容ハッシュを照合）。
読めない／壊れたマニフェストはスキップして他を続けますが、スキップがあれば終了コードは失敗になります。
完了時に所要時間を表示します。

### `care` — 整合確認のあと一括圧縮する

```powershell
snapline --target C:\work care
snapline --background care
```

`verify` 相当の確認のあと、参照オブジェクトのうち圧縮が効くものを **zstd レベル 5** で差し替えます。
ハッシュは変えません。普段の `snap` で溜まった raw を、たまにまとめて小さくする用途です。
壊れたマニフェストの扱いと終了コードは `verify` と同じです。所要時間も表示します。

### `config` — 現在の設定を表示する

```powershell
snapline --target C:\work config
```
...
`snap` / `care` / `restore` / `verify` に付けて使う。
ゲームやブラウジング中に、表の処理を邪魔しにくくする。
ペース（待機）だけが変わり、包含・除外・検証ルールは通常と同じ。

```powershell
snapline --background snap -m "while gaming"
snapline --background care
snapline --background restore a1b2c3d4e5f6 D:/restored-work
snapline --background verify

# しきい値を明示する場合
snapline --background --cpu-busy-percent 60 snap
```

`init` / `log` / `tree` / `find` / `config` / `install` に `--background` を付けるとエラーになる（無視して続行しない）。
...
書き込み系コマンドが `write.lock` を取るとき、`tmp/` に残った一時ファイルは自動で掃除します。
...
変更は次の `snap` から反映されます。専用の設定変更コマンドはまだありません。
